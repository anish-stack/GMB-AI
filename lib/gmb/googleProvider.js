...
export class GoogleGMBProvider {
  ...
  /**
   * Update a live post already on Google. postName is the full resource name
   * returned by createPost (e.g. "accounts/123/locations/456/localPosts/789"),
   * stored as gmb_posts.external_id.
   */
  async updatePost(clientId, postName, post) {
    const payload = {
      ...
      summary: [post.title, post.description].filter(Boolean).join("\n\n").slice(0, 1500),
      topicType: "STANDARD",
      ...(post.cta_url ? { callToAction: { actionType: "LEARN_MORE", url: post.cta_url } } : {}),
      ...(post.public_image_url ? { media: [{ mediaFormat: "PHOTO", sourceUrl: post.public_image_url }] } : {}),
    };
    ...
    return { external_id: updated.name || postName, is_mock: false, updated_at: new Date().toISOString() };
  }

  /** Delete a live post from Google. postName is the full localPosts resource name. */
