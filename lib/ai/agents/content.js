...
import { cleanPostText } from "../textPolicy.js";
...
export async function runContentAgent(kb, input, ctx = {}) {
  ...
  const title = cleanPostText(clean(data?.title, 120)).replace(/[.!]+$/, "") || `${input.topic}`;
  const description = cleanPostText(clean(data?.description, 1400)) || fallbackBody(kb, input);
