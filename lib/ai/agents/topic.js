...
export async function runTopicAgent(kb, { keywords, postType, presetTopic }, ctx = {}) {
  ...
  const recent = new Set((kb.recent_topics || []).map((t) => String(t).toLowerCase()));
  let topic = String(data?.topic || "").trim();
  const alternatives = Array.isArray(data?.alternatives) ? data.alternatives.map(String) : [];

  // Avoid repeating a recently used topic
  if (!topic || recent.has(topic.toLowerCase())) {
    const alt = alternatives.find((a) => !recent.has(a.toLowerCase()));
    topic = alt || rotateService(kb, recent);
  }
...
function rotateService(kb, recent) {
  const unused = kb.services.filter((s) => !recent.has(`${s}`.toLowerCase()));
