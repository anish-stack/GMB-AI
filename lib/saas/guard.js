...
import { maintenanceState, bypassesMaintenance } from "../system/maintenance.js";
...
export async function guard(_request, { permission = null, superAdmin = false } = {}) {
  ...
  if (m.enabled) {
    const ip = (_request?.headers?.get?.("x-forwarded-for") || "").split(",")[0].trim() || _request?.headers?.get?.("x-real-ip") || null;
    if (!bypassesMaintenance(m, { session: ctx.session, ip })) {
      return {
        error: NextResponse.json(
          { error: m.message, code: "MAINTENANCE", meta: { title: m.title, eta: m.eta || null } },
