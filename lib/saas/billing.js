...
import { paymentReceivedEmail } from "../mail/templates.js";
...
export async function markInvoicePaid({ invoiceId, gateway = "MANUAL", gatewayPaymentId = null, gatewayOrderId = null, raw = null, actor = "system" }) {
  ...
  // Email the tenant a receipt - queued so the payment webhook / admin action
  // returns immediately (see lib/mail/queue.js).
  try {
    const tenant = await one("SELECT name, company_email FROM tenants WHERE id=?", [inv.tenant_id]);
    let planLabel = null;
    if (inv.type === "SUBSCRIPTION" && inv.subscription_id) {
    ...
    if (tenant?.company_email) {
      ...
      await queueEmail({ to: tenant.company_email, subject: tpl.subject, html: tpl.html, text: tpl.text, template: "payment", tenantId: inv.tenant_id });
    }
  } catch (err) {
    console.error("[billing] failed to queue payment email:", err.message);
  }
