...
export function otpEmail({ name, code, minutes, purpose = "LOGIN" }) {
  const heading = purpose === "RESET_PASSWORD" ? "Reset your password" : purpose === "SIGNUP" ? "Verify your email" : "Your sign-in code";
  const html = shell({
    title: heading,
...
export function gmbReportEmail({ businessName, days, totals, source, appUrl, reportUrl = null }) {
  const liveNote =
    source === "google_live"
  ...
  const html = shell({
    ...
    bodyHtml: `
      ...
      <p style="font-size:14px;color:#475569;line-height:1.7;margin:0 0 14px;">
        Here's how <strong>${escapeHtml(businessName)}</strong> performed on Google Search &amp; Maps. The full report (daily views, customer actions, reviews, search terms and next steps) is attached as a PDF.
      </p>
      ...
      ${reportUrl ? button(reportUrl, "Open the report") : appUrl ? button(appUrl, "View full dashboard") : ""}
    `,
  });
  return {
    subject: `Your Google Business Profile report - ${businessName}`,
    html,
    text: `${businessName}: ${totals.views} views, ${totals.clicks} clicks, ${totals.calls} calls over the last ${days} days. Full PDF report attached${reportUrl ? ` - or open it online: ${reportUrl}` : ""}.`,
  };
}
