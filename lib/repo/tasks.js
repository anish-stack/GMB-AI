...
export async function listTasks({ status = null, clientId = null, date = null, limit = 100 } = {}) {
  const where = [];
  const params = [];
  if (status) {
...
export async function getTask(id) {
  const row = await one(
    `SELECT t.*, c.business_name, c.city, c.state, c.business_category, c.phone, c.website,
       ...
       FROM ai_tasks t JOIN clients c ON c.id=t.client_id WHERE t.id=?`,
    [id]
  );
  if (!row) return null;
  ...
  task.executions = await query(
    "SELECT * FROM ai_executions WHERE task_id=? ORDER BY id ASC",
    [id]
  );
  task.timeline = await query(
    "SELECT * FROM employee_approvals WHERE task_id=? ORDER BY id ASC",
    [id]
  );
...
export async function editTask(taskId, session, fields) {
  const allowed = ["title", "description", "cta", "primary_keyword", "topic"];
  const patch = {};
...
export async function approveTask(taskId, session) {
  await update("ai_tasks", taskId, { status: TASK_STATUS.APPROVED });
  await logAction(taskId, session, "APPROVED");
}

export async function rejectTask(taskId, session, reason) {
 ...
 */
export async function publishTask(taskId, session) {
  ...
  for (const kw of keywords) {
    const existing = await one("SELECT id FROM keywords WHERE client_id=? AND keyword=? LIMIT 1", [
      task.client_id,
      kw,
    ]);
  ...
  // Prototype-only demo metrics so the analytics page has data
  if (provider.isMock) {
    for (let i = 0; i < 7; i++) {
      const d = new Date();
