...
import { publishProviderFor } from "../gmb/provider.js";
...
import { postPublishedEmail } from "../mail/templates.js";
...
export async function publishTask(taskId, session, tenantId = null) {
  ...
  const provider = await publishProviderFor(task.client_id);
  ...
  // Notify the GMB owner (the client's connected Google account, falling back
  // to the agency/tenant email) that their post is live. Queued, not awaited
  // inline, so publishing stays fast even if SMTP is slow (lib/mail/queue.js).
  try {
    const owner = await one(
      `SELECT c.business_name, c.google_email, t.company_email AS tenant_email
         FROM clients c JOIN tenants t ON t.id=c.tenant_id WHERE c.id=?`,
      [task.client_id]
    );
    const to = owner?.google_email || owner?.tenant_email;
    if (to) {
      const tpl = postPublishedEmail({
      ...
      await queueEmail({ to, subject: tpl.subject, html: tpl.html, text: tpl.text, template: "post-published", tenantId: task.tenant_id });
    }
  } catch (err) {
    console.error("[publishTask] failed to queue post-published email:", err.message);
  }
