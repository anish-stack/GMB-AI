...
export async function getClient(id) {
  const client = await one("SELECT * FROM clients WHERE id=?", [id]);
...
export async function updateClient(id, payload) {
  ...
  if (payload.target_keywords !== undefined) {
    for (const k of splitList(payload.target_keywords)) {
      ...
      if (!exists) {
        await insert("keywords", {
          client_id: id, keyword: k.toLowerCase(), kw_type: "PRIMARY", source: "CLIENT",
          relevance_score: 90, priority: "HIGH", reason: "Provided by client/admin",
        });
      }
    }
  }
